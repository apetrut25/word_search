document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 1. DOM & CONFIGURATION ---
    const GAME_VERSION = "1.2.0"; // Final polish and bug fixes
    ...
    // --- 7. WORD SELECTION & PROCESSING (FIXED) ---
    ...
    // THIS FUNCTION IS THE FIX for interactive highlighting
    function highlightLine(startCell, endCell) {
        ...
        if (start.r === end.r || start.c === end.c || Math.abs(dR) === Math.abs(dC)) {
            const steps = Math.max(Math.abs(dR), Math.abs(dC));
            const stepR = Math.sign(dR);
            const stepC = Math.sign(dC);
            for (let i = 0; i <= steps; i++) {
                const cell = document.querySelector(`[data-row='${start.r + i * stepR}'][data-col='${start.c + i * stepC}']`);
                if (cell) {
    ...
    // --- 8. HINT SYSTEM (UPDATED) ---
    function handleHintRequest(e) {
        const word = e.target.textContent;
        const hintCost = 75;
        if (gameState.foundWords.includes(word)) return;
        if (gameState.score < hintCost) { alert(`Not enough points! A hint costs ${hintCost} points.`); return; }
        sound.play('hint');
        gameState.score -= hintCost;
        gameState.currentLevelData.pointsEarned -= hintCost;
        if (gameState.currentLevelData) gameState.currentLevelData.hintsUsed++;
        updateStats();
        saveState();
        const location = gameState.wordLocations[word];
        if (location) {
            const hintCell = document.querySelector(`[data-row='${location.r}'][data-col='${location.c}']`);
            if (hintCell) {
                hintCell.classList.add('hint');
                // FIXED: Duration should match CSS animation
    ...
    function startLevel() {
        ...
        if (gameState.bibleMode) {
            ...
            // No longer setting wordListTitleEl
            let wordsWithVerses = [];
    ...
    newGameBtn.addEventListener('click', () => { if (gameState.foundWords.length === gameState.words.length) { gameState.level++; gameState.foundWords = []; startLevel(); } else { alert("Please complete the current puzzle first!"); } });
    skipBtn.addEventListener('click', () => { const skipCost = 100; if (gameState.score < skipCost) { alert(`You need at least ${skipCost} points to skip a puzzle!`); return; } if (confirm(`Are you sure you want to skip this puzzle? It will cost ${skipCost} points.`)) { saveHistory(gameState.currentLevelData); gameState.score -= skipCost; gameState.level++; gameState.foundWords = []; startLevel(); } });
